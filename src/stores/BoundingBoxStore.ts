...
import type { WheelEventInfo } from '$input/PointerHandlers/WheelHadler';
...
function updateBoundingBoxOnZoom(
	...
	if (current_scale < 1) {
		...
		const max_x = default_bbox.height;
		const max_y = default_bbox.width;
...
export function updateBoundindBox(wheel_event: WheelEventInfo) {
	...
	boundingBoxStore.update((bbox) => {
		return updateBoundingBoxOnZoom(default_bbox, bbox, wheel_event, new_scale);
	});
}
