import { cellsFromCoords, cellsToVarsName, PuzzleModel } from "$src/lib/Solver/solver_utils";
...
export function getCageVars(grid: Grid, constraint: CageToolI) {
...
export function simpleCageElement(model: PuzzleModel, element: ConstraintsElement, predicate: string) {
    let out_str = '';
    const constraints = element.constraints;
