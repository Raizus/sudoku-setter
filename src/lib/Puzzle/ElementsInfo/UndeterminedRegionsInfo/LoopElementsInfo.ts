...
function loopRegionSegmentCellsNotCoPrimeConstraint(model: PuzzleModel, tool: TOOLID) {
	const puzzle = model.puzzle;
	const grid = puzzle.grid;
	...
	for (const region of used_regions) {
		...
		const coords = '[' + cells.map((cell) => `(${cell.r},${cell.c})`).join(', ') + ']';
		const grid_vars = cellsToGridVarsStr(cells, VAR_2D_NAMES.BOARD);
		const name1 = `lrscncp_region_${region}_coords`;
		const name2 = `lrscncp_region_${region}_values`;
		out_str += `array[int] of tuple(int, int): ${name1} = ${coords};\n`;
		out_str += `array[int] of var int: ${name2} = ${grid_vars};\n`;
		...
		out_str += `constraint path_segment_cells_not_co_prime_p(${name1}, ${name2}, ${loop}, ${edges_h}, ${edges_v});\n`;
	}
...
export function cellCenterLoopElement(model: PuzzleModel, element: ConstraintsElement) {
	...
	// loop region segments
	const regions = grid.getUsedRegions();
	const loop_region_segments = VAR_2D_NAMES.CELL_CENTER_LOOP_REGION_SEGMENTS;
	out_str += `array[ROW_IDXS, COL_IDXS] of var int: ${loop_region_segments};\n`;
	for (const region of regions) {
		const cells = grid.getRegion(region);
		const coords = '[' + cells.map((cell) => `(${cell.r},${cell.c})`).join(', ') + ']';
		const name1 = `loop_segments_region_${region}_coords`;
		const name2 = `loop_segments_${region}_labels`;
		const segment_vars = cellsToGridVarsStr(cells, loop_region_segments);

		out_str += `array[int] of tuple(int, int): ${name1} = ${coords};\n`;
		out_str += `array[int] of var int: ${name2} = ${segment_vars};\n`;
		out_str += `constraint path_region_segment_p(${name1}, ${name2}, ${loop}, ${edges_h}, ${edges_v});\n`;
	}
	...
	const loop_region_segment_cells_not_co_prime =
		!!neg_constr[TOOLS.LOOP_REGION_SEGMENT_CELLS_NOT_CO_PRIME];
	if (loop_region_segment_cells_not_co_prime) {
		out_str += loopRegionSegmentCellsNotCoPrimeConstraint(
			model,
			TOOLS.LOOP_REGION_SEGMENT_CELLS_NOT_CO_PRIME
		);
	}
...
export const cellCenterLoopInfo: SquareCellElementInfo = {
	...
	negative_constraints: [
		...
		{
			toolId: TOOLS.LOOP_REGION_SEGMENT_CELLS_NOT_CO_PRIME,
			description:
				"Region borders divide the loop into segments. Within each segment, all digits must be 'pairwise prime' (that is, any two digits in the same segment share no common factor greater than 1)."
		},
		{
