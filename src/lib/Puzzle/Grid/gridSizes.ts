...
export function getRegionIndex(row: number, col: number, size: number | null): number | null {
	if (size === null) return null;

	const region_dims = getRegionSize(size);
	if (region_dims === null) return null;

	const [region_h, region_w] = [region_dims.h, region_dims.w];
	return Math.floor(row / region_h) * region_h + Math.floor(col / region_w);
}
